...
<div class = 'answer-box-container' v-for="name in names" :key="name.name">
...
<div id='feedback' v-if=clicked>
  ...
  <div  v-if=result>
    ...
    <img src = 'http://www.scalsys.com/backgrounds/doge-white-background/doge-white-background_844858.jpg'>
     <br>
       <router-link to='/hard'> <button> Next Level </button></router-link>
  </div>
    ...
    handleClick() {
      ...
      for (let i = 0; i < wrongBox.length; i++) {
        ...
        if (name[i].id !== images[i].id) {
          wrong.innerHTML = "<img height=15 src= 'https://upload.wikimedia.org/wikipedia/commons/thumb/5/5f/Red_X.svg/240px-Red_X.svg.png'>";
          wrongBox[i].append(wrong);
        } else if (name[i].id === images[i].id){
           wrong.innerHTML = "<img height=15 src= 'https://upload.wikimedia.org/wikipedia/commons/thumb/b/bd/Checkmark_green.svg/417px-Checkmark_green.svg.png'>";
          wrongBox[i].append(wrong);
        }
      }
...
.start-box, .answer-box-container{
 flex-basis: 25%;
 margin: 5px;
 width: 120px;
 /* margin: 10px; */
